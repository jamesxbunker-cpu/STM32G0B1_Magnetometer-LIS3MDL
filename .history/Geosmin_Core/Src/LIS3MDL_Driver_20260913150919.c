...
static LIS3MDL_Status_t LIS3MDL_RestartConversion(LIS3MDL_Handle_t *dev)
{
    /* Power-down, wait, back to continuous. Forces the internal
     * conversion engine to restart cleanly. Required after any ODR
     * or FAST_ODR change. */
    LIS3MDL_Status_t s;
    uint8_t ctrl3 = 0;

    s = LIS3MDL_ReadReg(dev, LIS3MDL_CTRL_REG3, &ctrl3);
    if (s != LIS3MDL_OK) return s;

    ctrl3 &= (uint8_t)~LIS3MDL_CTRL3_MD_MASK;

    s = LIS3MDL_WriteReg(dev, LIS3MDL_CTRL_REG3,
                         (uint8_t)(ctrl3 | LIS3MDL_CTRL3_MD_PD));
    if (s != LIS3MDL_OK) return s;

    HAL_Delay(10);

    s = LIS3MDL_WriteReg(dev, LIS3MDL_CTRL_REG3,
                         (uint8_t)(ctrl3 | LIS3MDL_CTRL3_MD_CONT));
    if (s != LIS3MDL_OK) return s;
...
LIS3MDL_Status_t LIS3MDL_SetODR(LIS3MDL_Handle_t *dev, LIS3MDL_ODR_t odr)
{
    ...
    return LIS3MDL_RestartConversion(dev);
}
