 ...
 * not a big fan of how I am currently doing it*/
 ...
 #define LIS3MDL_SPI_READ_BIT 0x80  // Read bit for SPI transactions
 #define LIS3MDL_SPI_AUTO_INC 0x40  // Auto-increment bit for SPI transactions
 ...
 * @note   SPI write: bit7 = 0 (write), bit6 = 0 (single byte).
 */
 ...
 * @note   SPI read: bit7 = 1 (read), bit6 = 0 (single byte).
 */
LIS3MDL_Status_t LIS3MDL_ReadReg(LIS3MDL_Handle_t *dev, uint8_t reg, uint8_t *value){
    ...
    uint8_t tx[2] = { (uint8_t)(reg | LIS3MDL_SPI_READ_BIT), 0 };
    uint8_t rx[2] = { 0 };
