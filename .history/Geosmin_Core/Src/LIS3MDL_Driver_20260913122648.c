 ...
 * @note   ST bits are in CTRL_REG1[0] (ST_X/Y) and CTRL_REG4[2] (ST_Z),
 *         but ST_X/Y share bit0 with MD_Z.  The safest approach is to
 *         set the ST field carefully, preserving MD_Z semantics:
 *         - In continuous mode, MD_Z bit0 must be 0 when ST is 0,
 *           or 1 to enable X/Y self-test.
 *         This is why self-test should only be run in a controlled
 *         sequence (per datasheet AN4602)
 */
